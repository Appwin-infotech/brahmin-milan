...
const buildCloseUpPhotoUrl = (filename) =>
  `${BASE_URL}/uploads/closeUpPhoto/${filename}`;

// Delete a close-up photo file from disk given its public URL
const deleteCloseUpPhotoFile = (photoUrl) => {
  try {
    if (!photoUrl) return;
    const filename = photoUrl.split("/").pop();
    const filePath = path.join(UPLOAD_ROOT, "closeUpPhoto", filename);
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
    }
  } catch (err) {
    console.error("Error deleting close-up photo file:", err.message);
  }
};
...
const createPersonalDetails = async (req, res) => {
  try {
    ...
    // 🔹 Handle closeUpPhoto upload via Multer (local disk) - 1 to 3 required
    // With upload.fields([{ name: "closeUpPhoto", maxCount: 3 }]),
    // req.files is an object keyed by fieldname: req.files.closeUpPhoto is the array
    const closeUpFiles = req.files?.closeUpPhoto || [];
    ...
    personalDetails.closeUpPhoto = closeUpFiles.map((file) =>
      buildCloseUpPhotoUrl(file.filename)
    );
...
const updatePersonalDetails = async (req, res) => {
  try {
    ...
    if (removePhotos) {
      ...
      toRemove.forEach((photoUrl) => deleteCloseUpPhotoFile(photoUrl));
    }

    // 🔹 Step 2: New files uploaded via Multer (local disk)
    // With upload.fields(...), req.files is an object keyed by fieldname
    const newCloseUpFiles = req.files?.closeUpPhoto || [];
    const newPhotoUrls = newCloseUpFiles.map((file) =>
      buildCloseUpPhotoUrl(file.filename)
    );
...
const deleteBiodataProfile = async (req, res) => {
  try {
    ...
    // 🔹 Delete close-up photo files from disk before removing the record
    const photosToDelete = exitsBiodata.personalDetails?.closeUpPhoto || [];
    photosToDelete.forEach((photoUrl) => deleteCloseUpPhotoFile(photoUrl));
