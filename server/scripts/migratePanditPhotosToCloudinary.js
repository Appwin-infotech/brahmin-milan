...
const Pandit = require("../models/pandit"); // adjust path/model name to match yours

database.connect();
cloudinaryConnect();

// Folder where multer originally saved files, relative to project root
// (adjust if pandit photos were saved to a different folder)
...
async function uploadOneFile(oldPath, folder, contextLabel, failures) {
    if (typeof oldPath !== "string") {
        console.warn(`Skipping non-string photo entry for ${contextLabel}:`, oldPath);
...
async function migrate() {
    // Match records that have either field set, using a cursor to avoid
    // any hidden batch-size limit on large collections.
    const query = {
        $or: [
    ...
    const totalMatching = await Pandit.countDocuments(query);
    console.log(`Found ${totalMatching} pandit records with photos set (processing via cursor)`);

    const cursor = Pandit.find(query).cursor();
    ...
    for (let record = await cursor.next(); record != null; record = await cursor.next()) {
        ...
        if (oldProfilePhoto && typeof oldProfilePhoto === "string" && !isAlreadyUploaded(oldProfilePhoto)) {
            newProfilePhoto = await uploadOneFile(
                oldProfilePhoto,
                "pandit_photos",
                { panditId: record.panditId, field: "profilePhoto" },
                failures
            );
        ...
        if (!oldAdditional) {
            ...
            console.warn(`Unexpected additionalPhotos type for panditId ${record.panditId}:`, typeof oldAdditional);
            failures.push({ panditId: record.panditId, reason: "unexpected additionalPhotos type" });
            oldAdditional = [];
        }
        ...
        for (const oldPath of oldAdditional) {
            const newUrl = await uploadOneFile(
                oldPath,
                "pandit_photos",
                { panditId: record.panditId, field: "additionalPhotos" },
                failures
            );
        ...
        if (changed) {
            try {
                ...
                await Pandit.updateOne({ _id: record._id }, { $set: update });
                migratedCount++;
            } catch (err) {
                console.error(`Failed to save panditId ${record.panditId}:`, err.message);
                failures.push({ panditId: record.panditId, reason: `save failed: ${err.message}` });
            }
        } else {
            skippedCount++;
        }
    ...
    if (failures.length) {
        console.log(`\nWriting failure details to pandit-migration-failures.json`);
        console.table(failures);
        fs.writeFileSync(
            path.join(__dirname, "pandit-migration-failures.json"),
