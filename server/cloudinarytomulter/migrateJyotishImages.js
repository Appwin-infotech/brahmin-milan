...
async function migrateField(value, label) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      const filename = `migrated-jyotish-${label}-${Date.now()}-${Math.round(Math.random() * 1e6)}${ext}`;
...
async function migrate() {
  ...
  const jyotishes = await Jyotish.find({});
  console.log(`Found ${jyotishes.length} jyotish records`);

  for (const doc of jyotishes) {
    const label = doc.jyotishId || doc._id;
    const update = {};
    ...
    if (Object.keys(update).length > 0) {
      ...
      await Jyotish.updateOne({ _id: doc._id }, { $set: update });
    }
  }
