...
const Jyotish = require("../models/jyotish"); // adjust path/model name to match yours

database.connect();
cloudinaryConnect();

// Folder where multer originally saved files, relative to project root
// (adjust if jyotish photos were saved to a different folder)
...
async function migrate() {
    ...
    const totalMatching = await Jyotish.countDocuments(query);
    console.log(`Found ${totalMatching} jyotish records with photos set (processing via cursor)`);

    const cursor = Jyotish.find(query).cursor();
    ...
    for (let record = await cursor.next(); record != null; record = await cursor.next()) {
        ...
        if (oldProfilePhoto && typeof oldProfilePhoto === "string" && !isAlreadyUploaded(oldProfilePhoto)) {
            newProfilePhoto = await uploadOneFile(
                oldProfilePhoto,
                "jyotish_photos",
                { jyotishId: record.jyotishId, field: "profilePhoto" },
                failures
            );
        ...
        if (!oldAdditional) {
            ...
            console.warn(`Unexpected additionalPhotos type for jyotishId ${record.jyotishId}:`, typeof oldAdditional);
            failures.push({ jyotishId: record.jyotishId, reason: "unexpected additionalPhotos type" });
            oldAdditional = [];
        }
        ...
        for (const oldPath of oldAdditional) {
            const newUrl = await uploadOneFile(
                oldPath,
                "jyotish_photos",
                { jyotishId: record.jyotishId, field: "additionalPhotos" },
                failures
            );
        ...
        if (changed) {
            try {
                ...
                await Jyotish.updateOne({ _id: record._id }, { $set: update });
                migratedCount++;
            } catch (err) {
                console.error(`Failed to save jyotishId ${record.jyotishId}:`, err.message);
                failures.push({ jyotishId: record.jyotishId, reason: `save failed: ${err.message}` });
            }
        } else {
            skippedCount++;
        }
    ...
    if (failures.length) {
        console.log(`\nWriting failure details to jyotish-migration-failures.json`);
        console.table(failures);
        fs.writeFileSync(
            path.join(__dirname, "jyotish-migration-failures.json"),
